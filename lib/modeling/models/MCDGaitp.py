...
class MCDGaitp(BaseModel):
    ...
    def build_network(self, model_cfg):
        ...
        self.gcn_networks = nn.Sequential(Spatial_Basic_Block(ske_in_c[0], ske_in_c[1],
                                          ...
                                          Spatial_Basic_Block(ske_in_c[3], ske_in_c[3],
                                                              spatial_kernel_size),
                                          Spatial_Basic_Block(ske_in_c[3], ske_in_c[4],
                                                              spatial_kernel_size),
                                          Spatial_Basic_Block(ske_in_c[4], ske_in_c[4],
                                                              spatial_kernel_size))
        ...
        self.set_block4 = nn.Sequential(BasicConv2d(img_in_c[3], img_in_c[4], 3, 1, 1),
                                        nn.LeakyReLU(inplace=True),
                                        BasicConv2d(img_in_c[4], img_in_c[4], 3, 1, 1),
                                        nn.LeakyReLU(inplace=True))       
        ...
        self.fc_bin = nn.Parameter(
            nn.init.xavier_uniform_(
                torch.zeros(self.part_img*2, img_in_c[4], self.hidden_dim)))  #
        self.fc_bin1 = nn.Parameter(
            nn.init.xavier_uniform_(
                torch.zeros(self.part_ske*2, ske_in_c[4], self.hidden_dim)))  
        

        # GCN
        self.st_sil = st_grm(in_channels=img_in_c[4],
                                 out_channels=img_in_c[4],
                                 kernel_size=[3, 1],
                                 num_nodes=self.part_img,
                                 )
        
        self.st_ske = st_grm(in_channels=ske_in_c[4],
                                 out_channels=ske_in_c[4],
                                 kernel_size=[3, 1],
    ...
    def forward(self, inputs):
        ...
        outs4 = self.set_block4(outs3)
        x_1_s = self.hp(outs4.permute(4, 0, 2, 1, 3), dim=0)  # (p, n, c, s)
                                                              # (n, c, s, p)   1, 2, 3, 0
